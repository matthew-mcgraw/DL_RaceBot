...
export_file_name = "C:\\Users\\mcgra\\OneDrive\\Documents\\CourseMaterials\\AdvancedRobotics\\AR_RaceCar\\RecordedData\\fast_xy_pos_export.csv"
...
class Learn2Drive(RobotSupervisorEnv):
    ...
    def export_xy_pos_data(self):
        data = list(zip(self.x_pos_list,self.y_pos_list))
        with open(export_file_name, mode='w',newline='') as csv_file:
    ...
    def get_observations(self):
        ...
        self.x_pos_list.append(x)
        self.y_pos_list.append(y)
...
while True:
    ...
    if done:
        print("SCORE: ", env.episode_score)
        if env.episode_score > 501:
            break
        else:
            env.episode_score = 0.0
            observation = env.reset()