...
export_file_name = "C:\\Users\\mcgra\\OneDrive\\Documents\\CourseMaterials\\AdvancedRobotics\\AR_RaceCar\\RecordedData\\xy_pos_export.csv"
...
class Learn2Drive(RobotSupervisorEnv):
    def __init__(self):
        ...
        lidar_min = np.ones(60) * 0.1
        lidar_max = np.ones(60) * 4
        print(lidar_max)
    ...
    def solved(self):
        if len(self.episode_score_list) > 200:  # Over 100 trials thus far
            if np.mean(self.episode_score_list[-200:]) > 229 and np.mean(self.episode_score_list[-200:]) < 235:  # Last 100 episodes' scores average value
                return True
        return False
...
agent.load("C:\\Users\\mcgra\\OneDrive\\Documents\\CourseMaterials\\AdvancedRobotics\\AR_RaceCar\\AR_RaceCar\\Actor-Critic_Agents\\Racer_MultiSpeed_01")
