...
class PPOAgent:
    ...
    def work(self, agent_input, type_="simple"):
        ...
        with no_grad():
            action_prob = self.actor_net(agent_input).cpu()
    ...
    def train_step(self, batch_size=None):
        ...
        for _ in range(self.ppo_update_iters):
            ...
            for index in BatchSampler(SubsetRandomSampler(range(len(self.buffer))), batch_size, False):
                ...
                self.writer.add_scalar('Action Loss', action_loss,self.training_step)
                ...
                self.writer.add_scalar('Value Loss', value_loss,self.training_step)
                self.training_step += 1

        # After each training step, the buffer is cleared
        del self.buffer[:]
    
    def log_episode_score(self,episode_score,episode_number):
        self.writer.add_scalar('Episode Score',episode_score,episode_number)
