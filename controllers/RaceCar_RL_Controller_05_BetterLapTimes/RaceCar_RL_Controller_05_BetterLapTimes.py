...
class Learn2Trade():
    def __init__(self):
        ...
        self.steps_per_episode = 400  # Max number of steps per episode
        self.episode_score = 0  # Score accumulated during an episode
        self.episode_score_list = []  # A list to save all the episode scores, used to check if task is solved


        obs_space_min = [-15, -15, -1, -1, -1, -math.pi] 
        obs_space_max = [15, 15, 1, 1, 1, math.pi] 
        print(obs_space_min)
        print(obs_space_max)
    ...
    def check_for_checkpoints(self):
        ...
        if chkPts[0] == 0:
            if y > 0 and self.prev_y > 0 and x > 0 and self.prev_x < 0:
                chkPts[0] = 1.0
                print(chkPts)
                self.episode_score += 1/super().getTime()
                print("Past first checkpoint!")
                self.prev_x = x
                self.prev_y = y
                return chkPts
            elif y > 10 and self.prev_y > 10 and x < self.prev_x and x > -9 and x < 9:
            ...
            if y < 0 and self.prev_y > 0 and x > 0 and self.prev_x > 0:
                chkPts[1] = 1.0
                print(chkPts)
                self.episode_score += 10/super().getTime()
                print("Past second checkpoint!")
                return chkPts
            elif x > 10 and self.prev_x > 10 and y > self.prev_y and y < 9 and y > -9:
                print("WRONG DIRECTION")
                self.wrong_direction = True
                return chkPts
            else:
            ...
            if y < 0 and self.prev_y < 0 and x < 0 and self.prev_x > 0:
                chkPts[2] = 1.0
                print(chkPts)
                self.episode_score += 20/super().getTime()
            ...
            if y > 0 and self.prev_y < 0 and x < 0 and self.prev_x < 0:
                chkPts[3] = 1.0
                print(chkPts)
                self.episode_score += 30/super().getTime()
            ...
            if y > 0 and self.prev_y > 0 and x > 0 and self.prev_x < 0:
                chkPts[4] = 1.0
                print(chkPts)
                self.episode_score += 40/super().getTime()
    ...
    def is_done(self):
        ...
        if self.completed_lap == True:
            self.episode_score += 10000/super().getTime()
            return True
    ...
    def solved(self):
        if len(self.episode_score_list) > 200:  # Over 100 trials thus far
            if np.mean(self.episode_score_list[-200:]) > 625:  # Last 100 episodes' scores average value
    ...
    def apply_action(self, action):
        ...
        if action == 0:
            ...
            self.set_velocity(self.DRIVE_MAX_SPEED*3/4)
            self.set_steering_angle(0)
        elif action == 2:
            self.set_velocity(self.DRIVE_MAX_SPEED*7/8)
            self.set_steering_angle(0)
        elif action == 3:
            self.set_velocity(0)
            self.set_steering_angle(self.STEER_MAX_ANGLE)
            self.episode_score -= 0.01
        else:
            self.set_velocity(0)
            self.set_steering_angle(-self.STEER_MAX_ANGLE)
            self.episode_score -= 0.01
...
agent = PPOAgent(number_of_inputs=env.observation_space.shape[0], number_of_actor_outputs=env.action_space.n, 
                    use_cuda=True, batch_size=64, actor_lr = 0.0001, critic_lr = 0.00015)
agent.load("C:\\Users\\mcgra\\OneDrive\\Documents\\CourseMaterials\\AdvancedRobotics\\AR_RaceCar\\AR_RaceCar\\Actor-Critic_Agents\\Racer_MultiSpeed_04_Faster")
...
while not solved and episode_count < episode_limit:
    ...
    if episode_count % 1000 == 0:
        agent.save("C:\\Users\\mcgra\\OneDrive\\Documents\\CourseMaterials\\AdvancedRobotics\\AR_RaceCar\\AR_RaceCar\\Actor-Critic_Agents\\Racer_MultiSpeed_05_Faster")
