...
class Learn2Drive(RobotSupervisorEnv):
    def __init__(self):
        ...
        self.obst_node = self.getFromDef("MovingObstacle")
        ...
        self.DRIVE_MAX_SPEED = 80
        self.STEER_MAX_ANGLE = math.radians(35)
        
        #lidar_rays = self.lidar.getRangeImage()
        self.num_lidar_rays = 60
        #print(lidar_rays)
        lidar_min = np.ones(self.num_lidar_rays) * 0.1
        lidar_max = np.ones(self.num_lidar_rays) * 4
        print(lidar_max)
    ...
    def get_obstacle_translation(self):
        trans_field = self.obst_node.getField("translation")
        values = trans_field.getSFVec3f()
    ...
    def get_default_observation(self):
        ...
        ox,oy,oz = self.get_obstacle_translation()
        rand_x = np.random.uniform(10.9,14.5)
        #print(rand_x)
        
        self.obst_node.getField("translation").setSFVec3f([rand_x,oy,oz])
    ...
    def is_done(self):
        ...
        if self.completed_lap == True:
            ...
            self.episode_score += 1000/super().getTime()
            return True
    ...
    def solved(self):
        if len(self.episode_score_list) > 200:  # Over 100 trials thus far
            ...
            if np.mean(self.episode_score_list[-200:]) > 225:  # Last 100 episodes' scores average value
                return True
        return False
    ...
    def apply_action(self, action):
        ...
        if action == 0:
            ...
            self.set_velocity(self.DRIVE_MAX_SPEED/2)
            self.set_steering_angle(0)
        elif action == 2:
...
agent.load("C:\\Users\\mcgra\\OneDrive\\Documents\\CourseMaterials\\AdvancedRobotics\\AR_RaceCar\\AR_RaceCar\\Actor-Critic_Agents\\Racer_MovingObs_03")
...
while not solved and episode_count < episode_limit:
    ...
    if episode_count % 1000 == 0:
        agent.save("C:\\Users\\mcgra\\OneDrive\\Documents\\CourseMaterials\\AdvancedRobotics\\AR_RaceCar\\AR_RaceCar\\Actor-Critic_Agents\\Racer_MovingObs_04")
